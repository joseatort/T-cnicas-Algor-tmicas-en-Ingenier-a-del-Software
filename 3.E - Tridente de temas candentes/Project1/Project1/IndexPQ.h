...
private:
    ...
    std::unordered_map<string, int> posiciones;   // un 0 indica que el elemento no está
...
public:
    ...
    void push(string e, T const& p) {
        if (posiciones.find(e) != posiciones.end() && posiciones.count(e) == 0)
